...
const Todo = () => {
    ...
    switch (active) {
              ...
              <div>
                <div className={classes.checkBox}>
                  <h3
                    key={i.id}
                    className={`h3Class ${
                      i.completed ? classes.LineThrou : ""
                    }`}
                  >
                    {i.title}&nbsp;{i?.date}
                  </h3>
                  <div
                    className={classes.editClass}
                  ...
                  <input
                    type="checkbox"
                    checked={i.completed}
                    onChange={(e) => handlecheckChange(e, i.id)}
                  ></input>
                  <div
                    className={`${classes.deleteButton} testClass`}
            ...
            {showModal && (
              ...
              >
                <Box sx={style}>
                  ...
                  />
                  <button onClick={() => handleSave(editTodo, startDate)}>
                    Save
                  </button>
                </Box>
              </Modal>
      ...
      case 2:
        return (
          <div>
            {todoList?.map((i) => {
              return i.completed ? (
                <h3 className={classes.LineThrou}>{i.title}</h3>
              ) : null;
            })}
          </div>
        );
      case 3:
        return (
          <div>
            {todoList?.map((i) => {
              return !i.completed ? <h3>{i.title}</h3> : null;
            })}
          </div>
        );
      default:
  ...
  const totalProgress = useMemo(() => {
    ...
    const completedTodo = tempTodo.filter((i) => i.completed).length;
    const x = completedTodo / tempTodo.length;
    return x * 100;
  }, [todoList]);
